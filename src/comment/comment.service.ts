...
import { notDeletedQuery, paginationQuery } from 'src/common/helper/prisma-queries.helper';

@Injectable()
export class CommentService implements ICommentService {
  constructor(private readonly prisma: PrismaService) {}

  async createComment(userId: string, createCommentDto: CreateCommentDto): Promise<CreateCommentResponse> {
  ...
  async findAll(findAllCommentsDto: FindAllCommentsDto): Promise<CommentsResponse> {
    const { page, perPage, entityId } = findAllCommentsDto;
  ...
  async update(id: string, userId: string, content: string): Promise<UpdateCommentResponse> {
    const comment = await this.prisma.comment.findFirst({
      where: { id, userId, ...notDeletedQuery },
    });

    if (!comment) {
      throw new NotFoundException(`Comment with ID ${id} not found or you don't have permission`);
    }
